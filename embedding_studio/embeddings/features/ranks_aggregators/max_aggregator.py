...
class MaxAggregator(RanksAggregator):
    def __init__(self, if_empty_value: float = 0.0):
        """Aggregator that returns the maximum value from a list of ranks.

        This aggregator selects the highest rank value from the provided ranks. If the ranks
    ...
    def _aggregate(self, ranks: Union[List[float], torch.Tensor]) -> float:
        """Calculates the maximum value from the provided ranks.

        :param ranks: list of floats or a tensor of subitems ranks
        :return: the maximum value from ranks, or if_empty_value if ranks is empty
        """
        if len(ranks) == 0:
    ...
    def _aggregate_differentiable(self, ranks: torch.Tensor) -> torch.Tensor:
        """Calculates the differentiable maximum value from the provided ranks tensor.

        Uses differentiable_extreme to ensure gradient flow during training.

        :param ranks: a tensor of subitems ranks
        :return: a tensor containing the maximum value that supports gradient computation
        """
        if len(ranks) == 0:
